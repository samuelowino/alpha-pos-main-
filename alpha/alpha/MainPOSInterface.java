...
import SystemSecurity.SessionLogsView;
...
import staffandEmployeeManagement.ViewEmployees;
import staffandEmployeeManagement.ClockInClockOutTransactions;
import staffandEmployeeManagement.EmployeeDataTransaction;
import managedocuments.*;
import systemsandmanagementrinterface.UsersInformationTransactions;
import systemsandmanagementrinterface.ViewUsers;

public class MainPOSInterface extends JFrame {
    
    private static JMenuItem viewEmployeesMenuItem;
    private static JMenuItem viewUsersMenuItem;
    private static JMenuItem systemLoggsMenuItem;
    ...
    public static JLabel companyFooterLabel;
    ...
    protected static JComboBox<String> selectItemsCombo;
    protected static JComboBox<String> selectCategoryCombo;
    ...
    protected static JLabel inventoryTotalLabel;
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------------
    //Clock in/out panels
    public static JPanel clockInAndOutViewPanel;
    ...
    public MainPOSInterface(String loggedInUser, String sessionID, Boolean visible) {
        super("Alpha Pharmacy POS     Version 1.2 Build ID:201510222201\t\t\t:  Sesion ID       " + sessionID);
        setSize(1400, 800);
        setLocation(0, 0);
        ...
        systemLoggsMenuItem = new JMenuItem("Session Logs");
        
        try {
            applicationIconImage = ImageIO.read(new File("C:\\Users\\user\\NetBeansProjects\\PharmacyPOS\\src\\appimages\\nature-grass-green-tree-tree-leaves-leaves-autumn-path-house-house-foliage-sun-rays-blur-background-nature-tree-wallpaper-widescreen-full-screen-widescreen-hd-wallpapers-background-wallpaper.jpg"));
            setIconImage(applicationIconImage);
        } catch (IOException ex) {
        }
        viewEmployeesMenuItem = new JMenuItem("View Employees");
        viewUsersMenuItem = new JMenuItem("View Users");
        backgroundImageLabel = new JLabel(new ImageIcon("C:\\Users\\user\\NetBeansProjects\\PharmacyPOS\\src\\appimages\\nature-grass-green-tree-tree-leaves-leaves-autumn-path-house-house-foliage-sun-rays-blur-background-nature-tree-wallpaper-widescreen-full-screen-widescreen-hd-wallpapers-background-wallpaper.jpg"));
        ...
        logOutMenu = new JMenu(loggedInUser);
        ...
        viewSalesMenuItem = new JMenuItem("View Sales");
        salesReportsMenu.add(viewSalesMenuItem);
        ...
        logOutMenu.setFont(new Font("Calibri", Font.BOLD, 14));
        ...
        splitPane.setRightComponent(new SystemHomeScreen());
        ...
        viewEmployeesMenuItem.addActionListener(e -> {
            replaceRightComponentOnSplitPane(new ViewEmployees());
            splitPane.setDividerLocation(1);
        ...
        // inveicePanel
        viewSalesMenuItem.addActionListener(e -> {
        ...
        //Replace Right splitPane views
        systemLoggsMenuItem.addActionListener(e -> replaceRightComponentOnSplitPane(new SessionLogsView("all")));
        ...
        viewUsersMenuItem.addActionListener(event -> {
            ViewUsers viewUsers = new ViewUsers();
            viewUsers.viewUserActivityButton.addActionListener(e -> {
                if (viewUsers.usersViewTable.getSelectedRow() == -1) {
                    JOptionPane.showMessageDialog(viewUsers.usersViewTable, "Kindly,\nSelect "
                            + "a user and try again...", "Alpha Pharmacy POS",
                            JOptionPane.WARNING_MESSAGE);
                    
                } else if (viewUsers.usersViewTable.getSelectedRow() > -1) {
                    
                    int row = viewUsers.usersViewTable.getSelectedRow();
                    Object value = viewUsers.usersViewTable.getValueAt(row, 3);
                    replaceRightComponentOnSplitPane(new SessionLogsView(value.toString()));
                }
            });
            replaceRightComponentOnSplitPane(viewUsers);
        });
    ...
    /**
     * Retrieves database values for this particular table model for The
     * relevant JTable
     *
     * @param sortCategory
     * @param tableModel
    ...
    public static final void retrievedNaturalySortedTableData(String sortCategory, String ascdesc, DefaultTableModel tableModel, String tableName, String databaseTableName) {
        try {
            ...
            try {
                //Execute query
                ResultSet sortDataResults = globalSqlStatementObject.executeQuery("SELECT * FROM  "
                        + databaseTableName + " ORDER BY " + sortCategory + " " + ascdesc + ";");
                //get metadata
    ...
    public static final void retrieveTableData(String queryString, DefaultTableModel tableModel, String tableName, String databaseTableName) {
        try {
            int row = tableModel.getRowCount();
            while (row > 0) {
                row--;
    ...
    public static void categoriesComboBoxEvents() {
    ...
    public static final void destroyMainUI() {
        //   setVisible(false);
    }
